...
This reference architecture shows a set of proven practices for running several Linux virtual machines (VMs) behind a load balancer, to improve availability and scalability.  

![[0]][0]

## Architecture

This architecture builds on the one shown in [Running a Linux VM on Azure][single vm]. The recommendations there also apply to this architecture.
