...
## Overview
Our [reference implementation](tailspin.md) is an ASP.NET Core 1.0 application. The application uses the built-in OpenID Connect middleware to perform the OIDC authentication flow. The following diagram shows what happens when the user signs in, at a high level.
...
> [!NOTE]
> See [Startup.cs](https://github.com/Azure-Samples/identity-management-for-multitenant-apps/blob/master/src/Tailspin.Surveys.Web/Startup.cs).
> 
> 
